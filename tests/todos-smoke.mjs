...
try {
  ...
  assert.equal(rows.length, 16, 'initial tasks seeded once');
  assert.equal(await page.locator('#task-total').textContent(), '16');
  ...
  await secondPage.waitForFunction(() => document.querySelectorAll('.task-card').length === 17);
  ...
  console.log('todos-smoke: ok', JSON.stringify({ seeded: 16, statusUpdate: true, createTask: true, xssSafe: true, mobile: true }));
} finally {
  await browser.close();
