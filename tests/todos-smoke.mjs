...
try {
  ...
  await page.waitForFunction(() => document.querySelectorAll('.task-card').length === 15);

  assert.equal(await page.locator('.column').count(), 5, 'five Kanban columns');
  assert.equal(rows.length, 15, 'initial tasks seeded once');
  assert.equal(await page.locator('#task-total').textContent(), '15');
  ...
  await page.waitForFunction(() => document.querySelectorAll('.task-card').length === 16);
  ...
  await secondPage.waitForFunction(() => document.querySelectorAll('.task-card').length === 16);
  ...
  console.log('todos-smoke: ok', JSON.stringify({ seeded: 15, statusUpdate: true, createTask: true, xssSafe: true, mobile: true }));
} finally {
  await browser.close();
