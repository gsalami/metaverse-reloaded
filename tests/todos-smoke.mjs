...
try {
  ...
  assert.equal(rows.length, 18, 'initial tasks seeded once');
  assert.equal(await page.locator('#task-total').textContent(), '18');
  ...
  await secondPage.waitForFunction(() => document.querySelectorAll('.task-card').length === 19);
  ...
  console.log('todos-smoke: ok', JSON.stringify({ seeded: 18, statusUpdate: true, createTask: true, xssSafe: true, mobile: true }));
} finally {
  await browser.close();
