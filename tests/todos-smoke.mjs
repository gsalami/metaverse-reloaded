...
try {
  ...
  await page.waitForFunction(() => document.querySelectorAll('.task-card').length === 8);

  assert.equal(await page.locator('.column').count(), 5, 'five Kanban columns');
  assert.equal(rows.length, 8, 'initial tasks seeded once');
  assert.equal(await page.locator('#task-total').textContent(), '8');
  ...
  await page.waitForFunction(() => document.querySelectorAll('.task-card').length === 9);
  ...
  await secondPage.waitForFunction(() => document.querySelectorAll('.task-card').length === 9);
  ...
  console.log('todos-smoke: ok', JSON.stringify({ seeded: 8, statusUpdate: true, createTask: true, xssSafe: true, mobile: true }));
} finally {
  await browser.close();
