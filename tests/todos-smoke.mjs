...
try {
  ...
  await page.waitForFunction(() => document.querySelectorAll('.task-card').length === 17);

  assert.equal(await page.locator('.column').count(), 5, 'five Kanban columns');
  assert.equal(rows.length, 17, 'initial tasks seeded once');
  assert.equal(await page.locator('#task-total').textContent(), '17');
  ...
  await page.waitForFunction(() => document.querySelectorAll('.task-card').length === 18);
  ...
  await secondPage.waitForFunction(() => document.querySelectorAll('.task-card').length === 18);
  ...
  console.log('todos-smoke: ok', JSON.stringify({ seeded: 17, statusUpdate: true, createTask: true, xssSafe: true, mobile: true }));
} finally {
  await browser.close();
