...
try {
  ...
  assert.equal(rows.length, 20, 'initial tasks seeded once');
  assert.equal(await page.locator('#task-total').textContent(), '20');
  ...
  await page.waitForFunction(() => document.querySelectorAll('.task-card').length === 21);
  ...
  await secondPage.waitForFunction(() => document.querySelectorAll('.task-card').length === 21);
  ...
  console.log('todos-smoke: ok', JSON.stringify({ seeded: 20, statusUpdate: true, createTask: true, xssSafe: true, mobile: true }));
} finally {
  await browser.close();
